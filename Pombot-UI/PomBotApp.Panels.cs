...
namespace Pombot_UI
{
    partial class PomBotAppForm
    {
        ...
        private void exportExcelBT_Click(object sender, EventArgs e)
        {
            SaveDataList();
        }
        private void SaveDataList()
        {
            using (SaveFileDialog filedial = new SaveFileDialog() { Filter = "CSV|*.csv", ValidateNames = true })
            {
                if (filedial.ShowDialog() == DialogResult.OK)
                {
                    using (System.IO.StreamWriter sw = new System.IO.StreamWriter(new System.IO.FileStream(filedial.FileName, System.IO.FileMode.Create), Encoding.UTF8))
                    {
                        StringBuilder sb = new StringBuilder();
                        sb.AppendLine("Hour, Action, Price");
                        foreach (ListViewItem item in bot1Operations.Items)
                        {
                            sb.AppendLine(string.Format($"{item.SubItems[0].Text}, {item.SubItems[1].Text},{item.SubItems[2].Text}"));
                        }
